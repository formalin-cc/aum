...
public enum HttpCodeEnum {
    ...
    UNPROCESSABLE_ENTITY            (422, "Unprocessable Entity",               "请求格式正确，但是由于实体参数含有语义错误，无法响应"),
    ...
    CODE_10XX_MODEL                 (10,  "",                                   ""),
    AUTH_EXPIRED                    (1000,"",                                   "认证到期"),
    TOKEN_ERR                       (1001,"",                                   "token无效"),
    VERIFICATION_FAILED             (1002,"",                                   "业务逻辑验证未通过"),
    MODEL_NOT_EXIST                 (1003,"",                                   "模型不存在"),
    LOGERROR                        (1004,"",                                   "用户名或密码错误"),
    JWT_ERR                         (1005,"",                                   "JWT内部错误")
    ;
    ...
    HttpCodeEnum(int code, String messageUS, String messageCN) {
        this.code = code;
        this.messageUS = messageUS;
        this.messageCN = messageCN;
    }

    public HttpCodeEnum getSimpleType(){
        for (HttpCodeEnum codeEnum : HttpCodeEnum.values()){
            if (codeEnum.code == code / 100)
                return codeEnum;
        }
        return this;
    }
